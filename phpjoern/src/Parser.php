...
function parse_arguments() {
  ...
  // Parse options
  $longopts  = ["help", "version", "format:", "nodes:", "relationships:", "out:", "count:", "exclude-dir:"];
  $options = getopt( "hvf:n:r:o:c:e:", $longopts);
  if( $options === FALSE) {
  ...
  if ( isset( $options['exclude-dir']) || isset( $options['e'])) {
      global $exclude_dirs;
      $exclude_dir = $options['exclude-dir'] ?? $options['e'];
      $exclude_dirs = explode(",", $exclude_dir);
  }
...
function print_help() {
  ...
  echo '  -e, --exclude-dir <dir1(,dir2)> Exclude some dirs', PHP_EOL;
}
 ...
 *         was an error.
 */
function parse_file( $path, $exporter, $exclude_dirs) : int {

  $finfo = new SplFileInfo( $path);
  echo "Parsing file ", $finfo->getPathname(), PHP_EOL;

  try {
    $ast = ast\parse_file( $path, $version = 40);
 ...
 */
function parse_dir( $path, $exporter, $exclude_dirs, $top = true) : int {
  ...
  while( false !== ($filename = readdir( $dhandle))) {
    ...
    foreach ($exclude_dirs as $exclude_dir) {
        if (strncmp($finfo->getPathname(), $exclude_dir, strlen($exclude_dir)) === 0) {
            $isExclude = true;
            echo $finfo->getPathname().":exclude:".$exclude_dir, PHP_EOL;
            break;
        }
    }
    if ($isExclude) {
    ...
    if( $finfo->isFile() && $finfo->isReadable() && in_array( strtolower( $finfo->getExtension()), ['php','inc','phar']))
      $found[] = parse_file( $finfo->getPathname(), $exporter, $exclude_dirs);
    else if( $finfo->isDir() && $finfo->isReadable() && $filename !== '.' && $filename !== '..')
      if( -1 !== ($childdir = parse_dir( $finfo->getPathname(), $exporter, $exclude_dirs, false)))
        $found[] = $childdir;
  }
...
if( is_file( $path)) {
  ...
  parse_file( $path, $exporter, $exclude_dirs);
}
elseif( is_dir( $path)) {
  try {
    if( $format === Exporter::GRAPHML_FORMAT)
  ...
  parse_dir( $path, $exporter, $exclude_dirs);
}
else {
