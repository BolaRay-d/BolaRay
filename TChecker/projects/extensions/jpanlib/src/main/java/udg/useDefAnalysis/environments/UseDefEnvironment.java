...
public class UseDefEnvironment
{
			...
			Collection<String> symbols)
	{
		...
		if (astProvider instanceof ASTNodeASTProvider) {
			ASTNodeASTProvider nodeProvider = (ASTNodeASTProvider) astProvider;
			ASTNode node = nodeProvider.getASTNode();
			if (node instanceof Expression) {
				isDef = ((Expression) node).isForEachKeyOrValue();
			}
		}
		return createDefOrUseForSymbols(symbols, isDef);
	}
