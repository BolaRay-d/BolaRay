...
public abstract class ASTDefUseAnalyzer
{
	 ...
	 */
	protected void traverseAST(ASTProvider astProvider, boolean isCompact)
	{
		UseDefEnvironment env = createUseDefEnvironment(astProvider, isCompact);
		...
		for (int i = 0; i < numChildren; i++)
		{
			...
			if( env.shouldTraverse(childProvider))
				traverseAST(childProvider, env.isCompact());
	 ...
	 */
	protected abstract UseDefEnvironment createUseDefEnvironment(ASTProvider astProvider, boolean isCompact);
