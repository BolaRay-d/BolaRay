...
public class PHPCSVEdgeInterpreter implements CSVRowInterpreter
{
		...
		throws InvalidCSVFile
	{
		...
		switch (type)
		{
			...
			case PHPCSVNodeTypes.TYPE_NULLABLE_TYPE:
				errno = handleNullAbleType((NullAble)startNode, endNode, childnum);
				break;
	...
	private int handleFunction( FunctionDef startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			...
			case 3: // returnType child: either Identifier or NULL node
				...
				else if (endNode instanceof NullAble) {
					startNode.setReturnType(((NullAble)endNode).getTypeChild());
					startNode.setReturnNullable(true);
				}
				else {
					//startNode.addChild(endNode);
	...
	private int handleNullAbleType( NullAble startNode, ASTNode endNode, int childnum)
	{
		int errno = 0;
		...
		{
			case 0: // type child
				startNode.setTypeChild((Identifier)endNode);
				break;
	...
	private int handleCatch( CatchStatement startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // exception child: Identifier node
				...
				else if (endNode instanceof IdentifierList) {
