...
public class PHPCGFactory {
	...
	private static void createSpiderEdges(CG cg) {
		...
		for(Long ildId: toTopLevelFile.includeLoc) {
			...
			if (require instanceof IncludeOrEvalExpression) {
				Expression expression = ((IncludeOrEvalExpression) require).getExpression();
				if (expression instanceof StringExpression) {
					String topLevelFile = ((StringExpression) expression).getEscapedCodeStr();
					if (!topLevelFile.startsWith("./")) {
						topLevelFile = dir + "/" + topLevelFile;
						if (topLevelFunctionDefs.containsKey(topLevelFile)) {
							Long targetID = topLevelFunctionDefs.get(topLevelFile);
							call2mtd.add(ildId, targetID);
						}
					}
				}
			}
	...
	private static void createFunctionCallEdges(CG cg) {
		...
		for(CallExpressionBase functionCall : functionCalls) {
			...
			//we don't know the function name
				...
				ParseVar parsevar = new ParseVar();
				if (functionCall.getTargetFunc() == null) {
					System.err.println("Null function call target at node id " + functionCall.getNodeId() + "!");
					continue;
				}
		...
		for(Long caller: call2mtd.keySet()) {
			...
			for(Long mtd: call2mtd.get(caller)) {
				...
				if(ASTUnderConstruction.idToNode.get(caller) instanceof CallExpressionBase) {
					callerNode = new CGNode((CallExpressionBase) ASTUnderConstruction.idToNode.get(caller), false);
				}
				else if(ASTUnderConstruction.idToNode.get(caller) instanceof IncludeOrEvalExpression) {
					callerNode = new CGNode((IncludeOrEvalExpression) ASTUnderConstruction.idToNode.get(caller), false);
				}
				CGNode calleeNode = new CGNode((FunctionDef) ASTUnderConstruction.idToNode.get(mtd), true);
