...
public class PHPCFGFactory extends CFGFactory {
	...
	public static CFG newInstance(ForEachStatement forEachStatement)
	{
		try
		{
			...
			CFGNode valueContainer = new ASTNodeContainer(forEachStatement.getValueExpression());
			if (forEachStatement.getKeyExpression() != null) {
				CFGNode keyContainer = new ASTNodeContainer(forEachStatement.getKeyExpression());
				forEachBlock.addVertex(keyContainer);
				forEachBlock.addEdge(conditionContainer, keyContainer);
				forEachBlock.addVertex(valueContainer);
				forEachBlock.addEdge(keyContainer, valueContainer);
			}
			else {
				forEachBlock.addVertex(valueContainer);
				forEachBlock.addEdge(conditionContainer, valueContainer);
			}

			CFG forEachBody = convert(forEachStatement.getStatement());

			forEachBlock.mountCFG(valueContainer, conditionContainer,
